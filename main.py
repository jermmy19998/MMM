...
### Model Parameters.
parser.add_argument('--model_type',      type=str, choices=['mmm','snn', 'amil', 'mcat', 'motcat', 'mome','cmta'], 
...
parser.add_argument('--opt',             type=str,
                    choices=['adam', 'sgd'], default='adamw')
...
parser.add_argument('--main_log', type=str, default='',help='Main Log')
...
exp_code = str(args.split_dir) + str(args.exp_name) + str(args.exp_code) + '_microb{}'.format(args.bs_micro) + '_s{}'.format(args.seed)
...
def main(args):
    ...
    # Initialize wandb first, before any training
    if args.wandb:
        current_time = time.strftime('%Y-%m-%d %H:%M:%S')
        print(f"{Fore.RED}[{current_time}] running with {args.model_type} {args.mode}")
        ...
        wandb.save(f'/mnt/raid/zanzhuheng/working/prostate/MoME-master/models/model_mmm.py',policy='now')
        wandb.save(f'/mnt/raid/zanzhuheng/working/prostate/MoME-master/main.py',policy='now')
        wandb.save(f'/mnt/raid/zanzhuheng/working/prostate/MoME-master/utils/core_utils.py',policy='now')
        wandb.save(f'/mnt/raid/zanzhuheng/working/prostate/MoME-master/trainer/mb_trainer.py',policy='now')
        wandb.save(f'/mnt/raid/zanzhuheng/working/prostate/MoME-master/utils/utils.py',policy='now')
    ...
    for i in folds:
        ...
        if args.task_type == 'survival':
            summary_results, print_results, epoch_max_c_index = train(datasets, i, args) ### new
    ...
    if args.wandb:
        wandb.log({
            "Avg C-Index": result_cindex.mean(),
            "Std (Population)": result_cindex.std(),
            "Std (Sample)": result_cindex.std(ddof=1),
        })
        wandb.finish()

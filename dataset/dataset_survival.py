...
class Generic_WSI_Survival_Dataset(Dataset):
    def __init__(self, csv_path = 'dataset_csv/ccrcc_clean.csv', mode = 'omic', apply_sig = False, 
        ...
        ### new
        missing_slides_ls = [
                             'TCGA-A7-A6VX-01Z-00-DX2.9EE94B59-6A2C-4507-AA4F-DC6402F2B74F.svs',
                             ...
                             'TCGA-55-8207-01Z-00-DX1.2dafc442-f927-4b0d-b197-cc8c5f86d0fc.svs',     
                             ]
    ...
    def return_splits(self, from_id: bool=True, csv_path: str=None):
        if from_id:
            ...
            assert csv_path 
            # csv_path = '/mnt/raid/zanzhuheng/working/prostate/data/splits_0_test.csv'
            # pdb.set_trace()
        ...
        if not self.use_h5:
            if self.data_dir:
                if self.mode == 'path':
                    ...
                    omic1 = torch.tensor(np.array(self.genomic_features[self.omic_names[0]].iloc[idx]))
                    omic2 = torch.tensor(np.array(self.genomic_features[self.omic_names[1]].iloc[idx]))
                    omic3 = torch.tensor(np.array(self.genomic_features[self.omic_names[2]].iloc[idx]))
                    omic4 = torch.tensor(np.array(self.genomic_features[self.omic_names[3]].iloc[idx]))
                    omic5 = torch.tensor(np.array(self.genomic_features[self.omic_names[4]].iloc[idx]))
                    omic6 = torch.tensor(np.array(self.genomic_features[self.omic_names[5]].iloc[idx]))
...
class Generic_Split(Generic_MIL_Survival_Dataset):
    def __init__(self, slide_data, metadata, mode, signatures=None, data_dir=None, label_col=None, patient_dict=None, num_classes=2):
        ...
        self.genomic_features = self.slide_data.drop(self.metadata, axis=1) # (384, 9071)
        # pdb.set_trace()
        ...
        if self.signatures is not None:
            ...
            for col in self.signatures.columns:
                ...
                omic = np.concatenate([omic+mode for mode in ['_mut', '_cnv', '_rnaseq']]) # 246
                omic = sorted(series_intersection(omic, self.genomic_features.columns)) # 78
